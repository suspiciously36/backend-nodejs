  ...
  async handleUserAgent(req, res, next) {
    try {
      ...
      if (!userAgentInfo) {
        ...
        // Updating access_token to previous created user-agent
  
      } else {
        // Updating access_token to already existed user-agent if found
        await UserAgent.update(
          {
            access_token: cookieAccessToken,
          },
          { where: { id: userAgentInfo.dataValues.id } }
        );
      }

      // if (
      //   userAgentInfo &&
      //   userLoggedIn.accessToken !== userAgentInfo.access_token
      // ) {
      //   await UserAgent.create({
      //     user_id: userLoggedIn.id,
      //     device_type: result.device.type,
